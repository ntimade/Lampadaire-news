...
class HomeController extends Controller
{
    ...
    {
        // Recent published pool, reused to backfill the homepage shelves.
        // The 48h auto-prune wipes every article — including the ones an
        // editor had flagged "à la une" / "populaire" — while the automated
        // publisher never sets those flags, so without a fallback the hero
        // and popular blocks go permanently empty a couple of days in.
        $recentPool = News::with(['category', 'auther'])
            ->activeEntries()
            ->withLocalize()
            ->orderBy('id', 'DESC')
            ->take(30)
        ...
        if ($heroSlider->count() < 5) {
            $heroSlider = $heroSlider
                ->concat($recentPool->whereNotIn('id', $heroSlider->pluck('id')))
                ->unique('id')
                ->take(7)
                ->values();
        }

        $recentNews = $recentPool->take(6);

        $popularNews = News::with(['category'])->where('show_at_popular', 1)
            ->activeEntries()->withLocalize()
            ->orderBy('updated_at', 'DESC')->take(4)->get();

        if ($popularNews->count() < 4) {
            $popularNews = $popularNews
                ->concat(
                    News::with(['category'])
                        ->activeEntries()
                        ->withLocalize()
                        ->whereNotIn('id', $popularNews->pluck('id'))
                        ->orderBy('views', 'DESC')
                        ->orderBy('id', 'DESC')
                        ->take(4)
                        ->get()
    ...
    public function news(Request $request)
    {
        ...
        $news = $news->activeEntries()->withLocalize()->orderByDesc('id')->paginate(20);
