                ...
                <form action="{{ route('admin.news.update', $news->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        @if (canAccess(['news status', 'news all-access']))
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <div class="control-label">Inclure dans le journal</div>
                                    <label class="custom-switch mt-2">
                                        <input {{ $news->show_at_journal == 1 ? 'checked' : '' }} value="1"
                                            type="checkbox" name="show_at_journal" class="custom-switch-input">
                                        <span class="custom-switch-indicator"></span>
                                    </label>
                                    <small class="text-muted d-block">Sera proposé dans le journal imprimé téléchargeable.</small>
                                </div>
                            </div>

                            <div class="col-md-3">
                                <div class="form-group">
                                    <div class="control-label">À la Une du journal</div>
                                    <label class="custom-switch mt-2">
                                        <input {{ $news->is_journal_lead == 1 ? 'checked' : '' }} value="1"
                                            type="checkbox" name="is_journal_lead" class="custom-switch-input">
                                        <span class="custom-switch-indicator"></span>
                                    </label>
                                    <small class="text-muted d-block">Article vedette en première page — un seul possible, décoche automatiquement les autres.</small>
                                </div>
                            </div>

                            <div class="col-md-3">
                                <div class="form-group">
                                    <div class="control-label">Publier sur Facebook</div>
                                    <label class="custom-switch mt-2">
                                        <input {{ $news->post_to_facebook == 1 ? 'checked' : '' }} value="1"
                                            type="checkbox" name="post_to_facebook" class="custom-switch-input">
                                        <span class="custom-switch-indicator"></span>
                                    </label>
                                    @if ($news->posted_to_facebook_at)
                                        <small class="text-muted d-block">Déjà publié le {{ \Carbon\Carbon::parse($news->posted_to_facebook_at)->locale('fr')->translatedFormat('j F Y à H:i') }}.</small>
                                    @else
                                        <small class="text-muted d-block">Priorité sur la publication automatique dans la rubrique — sinon un article est choisi automatiquement.</small>
                                    @endif
                                </div>
                            </div>
                        @endif
